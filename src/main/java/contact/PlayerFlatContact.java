...
public class PlayerFlatContact extends ContactManager {
    ...
    public double findDistanceToBeX() {
        if (player.getCenterX() >= body.getCenterX())
            return player.getCenterX() - player.getRadius() + player.getxMove() - body.getCenterX() - body.getWidth() / 2 + 0.5;
        if (player.getCenterX() < body.getCenterX())
            return body.getCenterX() - body.getWidth() / 2 - player.getCenterX() - player.getRadius() - player.getxMove() + 1.5;
        return 1;
    }

    public double findDistanceToBeY() {
        if (player.getCenterY() >= body.getCenterY())
            return player.getCenterY() - player.getRadius() + player.getyMove() - body.getCenterY() - body.getHeight() / 2 + 6.5;
        if (player.getCenterY() < body.getCenterY())
            return body.getCenterY() - body.getHeight() / 2 - player.getCenterY() - player.getRadius() - player.getyMove() + 5.5;
        return 1;
    }
