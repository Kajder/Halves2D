...
public class MyMath{
	...
	public static double angleFromComponents(double componentX, double componentY){
		if ((Math.signum(componentX)>0)&&(Math.signum(componentY)>0))return Math.atan(componentY/componentX)-Math.PI/2;
		if ((Math.signum(componentX)<0)&&(Math.signum(componentY)>0))return Math.atan(componentY/componentX)+Math.PI/2;
		if ((Math.signum(componentX)<0)&&(Math.signum(componentY)<0))return Math.atan(componentY/componentX)+Math.PI/2;
		if ((Math.signum(componentX)>0)&&(Math.signum(componentY)<0))return Math.atan(componentY/componentX)-Math.PI/2;
	...
	boolean flag = false;	
	if ((distanceTwoPoints(P1,P2)>distanceTwoPoints(P,P1))&&(distanceTwoPoints(P1,P2)>distanceTwoPoints(P,P2))) flag=true;
