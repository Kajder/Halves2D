...
public class ClockIndicator {
    ...
    public void tick() {
        ...
        if ((hasBallP1) || ((ball.getV() < 0.25) && (ballVsPlayer1.getBall().getY() + ball.getRadius() / 2 > (handler.getHeight() / 2 + 5)))) {
            isCountedP1 = true;
            isCountedP2 = false;
        }
        if ((hasBallP2) || ((ball.getV() < 0.25) && ballVsPlayer2.getBall().getY() + ball.getRadius() / 2 < (handler.getHeight() / 2 - 5))) {
            isCountedP1 = false;
            isCountedP2 = true;
        }
        ...
        //countdown starts
        if ((wasCountedP1 != isCountedP1) || (wasCountedP2 != isCountedP2)) {
            ticksPassed = startingPoint * handler.getFPS();//if counted player switched
        }

        //countdown restarts
        if (isCountedP1 && ballInBox2) {
            ticksPassed = startingPoint * handler.getFPS();
        }
        if (isCountedP2 && ballInBox1) {
            ticksPassed = startingPoint * handler.getFPS();
        }
    ...
    public void render(Graphics2D g2d) {
        //whiteFace
        if ((isCountedP1 == false) && (isCountedP2 == false)) {
            g2d.drawImage(whiteFace, x, y + 20, width, height * 747 / 1000, null);
        }
        //blueFace
        if (isCountedP1) {
            g2d.drawImage(blueFace, x, y + 20, width, height * 747 / 1000, null);
        }
        //redFace
        if (isCountedP2) {
            g2d.drawImage(redFace, x, y + 20, width, height * 747 / 1000, null);
        }
        //clockFace
    ...
    public Path2D calculatePath() {
        ...
        path.lineTo(X + R * Math.sin(alpha), Y + R * Math.cos(alpha));
        ; //point B
