...
public class BallFlatContact extends ContactManager{
	...
	public void deflectBall(){
		...
		if (isHorizontal==true) {
			if ((alpha >= (Math.PI/2)) && (alpha <= (3*Math.PI/2))) {gamma = 2*alpha - Math.PI;}
			else {gamma = 2*alpha + Math.PI;}
		}
		if (isHorizontal==false) gamma = 2*alpha;
	...
	if ((findDistanceX()>=0)||(findDistanceY()>=0))contactAllowed = true;
	}
	...
	public double findDistanceX(){
		if (ballCenter.first>=bodyCenter.first) return ballCenter.first-ball.getRadius()-bodyCenter.first-body.getWidth()/2;
		if (ballCenter.first<bodyCenter.first) return bodyCenter.first-body.getWidth()/2-ballCenter.first-ball.getRadius();
		return 1;
	}
	public double findDistanceY(){
		if (ballCenter.second>=bodyCenter.second) return ballCenter.second-ball.getRadius()-bodyCenter.second-body.getHeight()/2;
		if (ballCenter.second<bodyCenter.second) return bodyCenter.second-body.getHeight()/2-ballCenter.second-ball.getRadius();
		return 1;
	}
