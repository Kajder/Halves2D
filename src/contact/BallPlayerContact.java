...
public class BallPlayerContact extends BallOrbContact {
  ...
  public void getInput() {
    ...
    if (player.getHasABall() == true && (forceInd <= (handler.getFPS() * ball.getforceMaxTime())) && (force)) { //
      ...
      if ((dirLeft) && (directionInd > (-ball.getAngleMaxTime() * handler.getFPS()))) {
        directionInd--;
      }
      if ((dirRight) && (directionInd < (ball.getAngleMaxTime() * handler.getFPS()))) {
        directionInd++;
      }
      if ((rotLeft) && (rotationInd < (ball.getwMaxTime() * handler.getFPS()))) {
        rotationInd++;
      }
      if ((rotRight) && (rotationInd > (-ball.getwMaxTime() * handler.getFPS()))) {
        rotationInd--;
      }
      if (shotCancellation) {
        forceInd = 0;
  ...
  public boolean checkIfcaught() {
    //System.out.println("Player " + player.getMode() + " may catch: " + player.getMayCatch());
    //System.out.println("Ball get cought by: " + ball.getCaughtBy() + " dribbling allowed"+ ball.getDribbleAllowed());
    //System.out.println("Ball Player distance: " + checkDistance());
    updateNumbers();
  ...
  public void deflectBall() {
    ...
    newXPrev2 = (xPrev2 * Math.cos(gamma) - xPrev2 * Math.sin(gamma));
  ...
  public double checkDistance() {
    ...
    //przyczyna gubienia pi³ki
    double result = Math.sqrt((player.getCenterX() - ball.getX()-ball.getRadius()) * (player.getCenterX() - ball.getX()-ball.getRadius())
        + (player.getCenterY() - ball.getY()-ball.getRadius()) * (player.getCenterY() - ball.getY()-ball.getRadius()));
    return result;
  }

  public void checkFirstTouch() {
    if (!player.getHasABall()) {
      storeParameters();
    }
  ...
  public void tick() {
    if ((TrainingState.trainingPlayer != 0) && player.getCallBall()) {
      translateBall();
    }
    updateNumbers();
    checkFirstTouch();
  ...
  public void render(Graphics g) {
    ...
    if (firstTouch) {
      ...
      if (ballIndicatorSize % 2 != 0) {
        ballIndicatorSize++;
      }
      ballIndicatorCorrection = (int) ((50 - ballIndicatorSize) / 2);
      if (ballIndicatorCorrection % 2 != 0) {
        ballIndicatorCorrection++;
      }
    }
    if ((firstTouch) && (player.getMode() == 1)) {
      g.drawImage(Assets.ballIndicatorBig, (int) (handler.getWidth() * 0.925 + ballIndicatorCorrection / 2), (int) (handler.getHeight() * 0.83 + ballIndicatorCorrection / 2), ballIndicatorSize + ballIndicatorCorrection, ballIndicatorSize + ballIndicatorCorrection, null);
    }
    if ((firstTouch) && (player.getMode() == 2)) {
      g.drawImage(Assets.ballIndicatorBig, (int) (handler.getWidth() * 0.925 + ballIndicatorCorrection / 2), (int) (handler.getHeight() * 0.09 + ballIndicatorCorrection / 2), ballIndicatorSize + ballIndicatorCorrection, ballIndicatorSize + ballIndicatorCorrection, null);
    }
    ...
    if ((player.getMode() == 1) && showVelocity) {
      g.drawString("V=" + velocity + "m/s", (int) (handler.getWidth() * 0.9), (int) (handler.getHeight() * 0.75));
    }
    if ((player.getMode() == 2) && showVelocity) {
      g.drawString("V=" + velocity + "m/s", (int) (handler.getWidth() * 0.9), (int) (handler.getHeight() * 0.25));
    }
  ...
  public void setThunderVisible(boolean flag) {
    thunderVisible = flag;
  }

  public boolean getThunderVisible() {
    return thunderVisible;
  }
}
