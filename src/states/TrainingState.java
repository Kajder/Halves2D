...
public class TrainingState extends State{
	...
	public void tick() {
		...
		if (trainingPlayer==1){
			ballVsPlayer1.tick();
		if (tick<=1) ballVsPlayer1.translateBall();
		}
		if (trainingPlayer==2){
			ballVsPlayer2.tick();
		if (tick<=1) ballVsPlayer2.translateBall();
		}
		if (tick<=1) {tick++;}else{tick=2;}
	...
	public void manageScore(){
		if (((ball.getCenterY()<(postUL.getCenterY()-ball.getHeight()/2-2)) && (ball.getCenterX()>(postUL.getCenterX()+3)) 
			...
			if ((!player1.getHasABall())&&(trainingPlayer==1)) ballForPlayer1();
			if ((!player2.getHasABall())&&(trainingPlayer==2)) ballForPlayer2();
