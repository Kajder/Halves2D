...
public class KeyManager implements KeyListener{
	...
	public boolean P1_up, P1_down, P1_left, P1_right, P1_force, P1_dirLeft, P1_dirRight, P1_rotLeft, P1_rotRight, P1_cancelation, P1_sprint, P1_dropOrCallBall,
	P2_up, P2_down, P2_left, P2_right, P2_force, P2_dirLeft, P2_dirRight, P2_rotLeft, P2_rotRight, P2_cancelation, P2_sprint, P2_dropOrCallBall;
	...
	public void tick(){
		...
		//System.out.println("number of all recognized controllers (Xbox and non-Xbox): "+Controllers.getControllerCount());
		 ...
		 P1_cancelation = keys[KeyEvent.VK_B];
		...
		//using flags to skip default -1 value of Xbox pad axes
			 //P1 moving up
			 if ((controller1.getAxisValue(0)!=-1) && (controller1.getPovY()!=-1)) releaseFlagP1Up=true;
			 if (releaseFlagP1Up==true){
				 if ((controller1.getAxisValue(0)<-deadZone1)) controller1axisY = controller1.getAxisValue(0); //Left analog
				 if ((controller1.getPovY()<-deadZone1)) P1_up = true; //D-Pad  
			 } 
				 if (keys[KeyEvent.VK_W]) P1_up = true; //keyboard
			//P1 moving down
			if ((controller1.getPovY()>deadZone1)|| keys[KeyEvent.VK_S]) P1_down = true; ///D-Pad / keyboard
			if (controller1.getAxisValue(0)>deadZone1) controller1axisY = controller1.getAxisValue(0); //Left analog	 
				 //System.out.println("Pad1: LA Y value: "+controller1.getAxisValue(0));
				 //System.out.println("Pad1: D-Pad Y value: "+controller1.getPovY());
			//P1 moving left 
			if ((controller1.getAxisValue(1)!=-1) && (controller1.getPovX()!=-1)) releaseFlagP1Left=true;
			if (releaseFlagP1Left==true){
				if (controller1.getAxisValue(1)<-deadZone1)controller1axisX = controller1.getAxisValue(1);
				if (controller1.getPovX()<-deadZone1)  P1_left = true; //D-Pad	 
			}
			if (keys[KeyEvent.VK_A]) P1_left = true; //keyboard
			//P1 moving right 
			if ((controller1.getPovX()>deadZone1) || keys[KeyEvent.VK_D]) P1_right = true; //D-Pad / keyboard
			if (releaseFlagP1Left==true) controller1axisX = controller1.getAxisValue(1); //to make sure default -1 won't interfere with movement
				 ...
				 P1_cancelation = (controller1.isButtonPressed(0) || keys[KeyEvent.VK_B]); //button A
			...
			//P1 direction left   
				 if (controller1.getAxisValue(4)>deadZone1 ||(keys[KeyEvent.VK_T])) P1_dirLeft = true; //left trigger/keyboard
				 
			//P1 direction right  
				 if (controller1.getAxisValue(4)!=-1) releaseFlagP1RT=true;
				 if ((releaseFlagP1RT==false)&&(controller1.getAxisValue(4)==-1))releaseFlagP1RT=false;
				 
				 if (releaseFlagP1RT==true){
						 if (controller1.getAxisValue(4)<-deadZone1) P1_dirRight = true; //right trigger
					 }	
				 if (keys[KeyEvent.VK_Y]) P1_dirRight = true; //right trigger
		 ...
		 P2_cancelation = keys[KeyEvent.VK_M];
		...
		//P2 moving up
			 if ((controller2.getAxisValue(0)!=-1) && (controller2.getPovY()!=-1)) releaseFlagP2Up=true;
			 if (releaseFlagP2Up==true){
				 if (controller2.getAxisValue(0)<-deadZone2) controller2axisY = controller2.getAxisValue(0); 
				 if (controller2.getPovY()<-deadZone2) P2_up = true; //Left analog
			 }
				 if (keys[KeyEvent.VK_UP]) P2_up = true; //keyboard
		//P2 moving down
			if ((controller2.getPovY()>deadZone2) || keys[KeyEvent.VK_DOWN]) P2_down = true; //Left analog/D-Pad
			if (controller2.getAxisValue(0)>deadZone2) controller2axisY = controller2.getAxisValue(0);  
		//P2 moving left 
			if ((controller2.getAxisValue(1)!=-1) && (controller2.getPovX()!=-1)) releaseFlagP2Left=true;
			if (releaseFlagP2Left==true){
				 if (controller2.getPovX()<-deadZone2) P2_left = true; //Left analog
				 if (controller2.getAxisValue(1)<-deadZone2) controller2axisX = controller2.getAxisValue(1);
			 }	 
			if (keys[KeyEvent.VK_LEFT]) P2_left = true; //keyboard
		//P2 moving right
			if ((controller2.getPovX()>deadZone2) || keys[KeyEvent.VK_RIGHT]) P2_right = true; //D-Pad / keyboard
			if (releaseFlagP2Left==true) controller2axisX = controller2.getAxisValue(1); //Left analog
		 
		 P2_force = (controller2.isButtonPressed(1) || keys[KeyEvent.VK_L]); //button X
		 P2_cancelation = (controller2.isButtonPressed(0) || keys[KeyEvent.VK_M]); //button A
		 ...
		 //P2 direction left 
		 if (controller2.getAxisValue(4)>deadZone2 || keys[KeyEvent.VK_U]) P2_dirLeft = true;//P2_dirLeft = true; //left trigger
		//P2 direction right
		 ...
		 if (releaseFlagP2RT==true){
				 if (controller2.getAxisValue(4)<-deadZone2) P2_dirRight = true;//P2_dirRight = true; //right trigger
			 }	
	...
	public float getController2axisY() { return controller2axisY; }
	@Override
