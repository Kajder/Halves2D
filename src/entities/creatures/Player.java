...
public class Player extends Creature{
	...
	public void checkIfRunning(){
		isRunning=false;
		if ((x!=prevXdouble)||(y!=prevYdouble)) isRunning=true;
		prevXdouble=x;
		prevYdouble=y;
	}
	...
	@Override
	public void tick() {
		if(stamina<staminaInitial)stamina+=staminaRebuilt;
		if(stamina<=150){staminaForceFactor=staminaForceFactorMin;}else{staminaForceFactor=staminaForceFactorMax;}
		//if (mode==1) System.out.println("stamina P1: "+stamina);
		//if (mode==2) System.out.println("stamina P2: "+stamina);
			getInput();
			move();
			if (getHasABall()==true) {Ball.speed=speed;
	...
	public void walk(){
		if (mode==1) walkPlayer = walkPlayer1;
		if (mode==2) walkPlayer = walkPlayer2;
		prevX=currX;
		prevY=currY;
		currX=(int) x;
		currY=(int) y;
		...
		if (walkPlayer.getIndex()==14)walkPlayer.setIndex(0);
	...
	private void getInput(){
		sprintPrev=sprint;
		adaptMode();
		if (mode==1) playerAngle=Math.PI;
		if (mode==2) playerAngle=0;
		xMove = 0;
		yMove = 0;
//SPEED
...
//KEYBOARD
		...
		if (up) playerAngle=Math.PI;			
		//keyboard and D-Pad down
		if (down) playerAngle=0;
		//keyboard and D-Pad right
		if (right) playerAngle=Math.PI/2;
		//keyboard and D-Pad left
		if (left) playerAngle=3*Math.PI/2;
		if (up&&right) playerAngle=Math.toRadians(135);
		if (up&&left) playerAngle=Math.toRadians(225);
		if (down&&right) playerAngle=Math.toRadians(45);
		if (down&&left)	playerAngle=Math.toRadians(-45);
...
//ANALOG
		...
		if (KeyManager.controller1!=null){
			...
			if (mode==1){
				if (c1x<deadZone1 && c1x>-deadZone1 && c1y<deadZone1 && c1y>-deadZone1) speed = 0;

				if (c1x<deadZone1 && c1x>-deadZone1 && c1y<deadZone1 && c1y>-deadZone1) playerAngle = Math.PI;
				if ((c1x<deadZone1 && c1x>-deadZone1 && c1y<-deadZone1)||up) playerAngle = Math.PI;
				if ((c1x<deadZone1 && c1x>-deadZone1 && c1y>deadZone1)||down) playerAngle = 0;
				if ((c1x<-deadZone1 && c1y<deadZone1 && c1y>-deadZone1)||left) playerAngle = -Math.PI/2;
				if (c1x<-deadZone1 && c1y<-deadZone1) playerAngle = -Math.PI/2+Math.asin(c1y/Math.sqrt(c1x*c1x+c1y*c1y));
				if (c1x<-deadZone1 && c1y>deadZone1) playerAngle = -Math.PI/2+Math.asin(c1y/Math.sqrt(c1x*c1x+c1y*c1y));
				if ((c1x>deadZone1 && c1y<deadZone1 && c1y>-deadZone1)||right) playerAngle = Math.PI/2;
				if (c1x>deadZone1 && c1y<-deadZone1) playerAngle = Math.PI/2-Math.asin(c1y/Math.sqrt(c1x*c1x+c1y*c1y));
				if (c1x>deadZone1 && c1y>deadZone1) playerAngle = Math.PI/2-Math.asin(c1y/Math.sqrt(c1x*c1x+c1y*c1y));
				if (up&&right) playerAngle=Math.toRadians(135);
				if (up&&left) playerAngle=Math.toRadians(225);
		...
		if (KeyManager.controller2!=null){
			...
			if (mode==2){
				if (c2x<deadZone2 && c2x>-deadZone2 && c2y<deadZone2 && c2y>-deadZone2) speed = 0;
				
			if (c2x<deadZone2 && c2x>-deadZone2 && c2y<deadZone2 && c2y>-deadZone2) playerAngle = 0;
			if ((c2x<deadZone2 && c2x>-deadZone2 && c2y<-deadZone2)||up) playerAngle = Math.PI;
			if ((c2x<deadZone2 && c2x>-deadZone2 && c2y>deadZone2)||down) playerAngle = 0;
			if ((c2x<-deadZone2 && c2y<deadZone2 && c2y>-deadZone2)||left) playerAngle = -Math.PI/2;
			if (c2x<-deadZone2 && c2y<-deadZone2) playerAngle = -Math.PI/2+Math.asin(c2y/Math.sqrt(c2x*c2x+c2y*c2y));
			if (c2x<-deadZone2 && c2y>deadZone2) playerAngle = -Math.PI/2+Math.asin(c2y/Math.sqrt(c2x*c2x+c2y*c2y));
			if ((c2x>deadZone2 && c2y<deadZone2 && c2y>-deadZone2)||right) playerAngle = Math.PI/2;
			if (c2x>deadZone2 && c2y<-deadZone2) playerAngle = Math.PI/2-Math.asin(c2y/Math.sqrt(c2x*c2x+c2y*c2y));
			if (c2x>deadZone2 && c2y>deadZone2) playerAngle = Math.PI/2-Math.asin(c2y/Math.sqrt(c2x*c2x+c2y*c2y));
				if (up&&right) playerAngle=Math.toRadians(135);
				if (up&&left) playerAngle=Math.toRadians(225);
...
//SETTERS and GETTERS	
	...
	public void setXMoveAllowedOrb(boolean permission){moveXallowedOrb = permission;}
	public void setYMoveAllowedOrb(boolean permission){moveYallowedOrb = permission;}
	public boolean getXMoveAllowedOrb(){return moveXallowedOrb;}
	public boolean getYMoveAllowedOrb(){return moveYallowedOrb;}
	public void setXMoveAllowedFlat(boolean permission){moveXallowedFlat = permission;}
	public void setYMoveAllowedFlat(boolean permission){moveYallowedFlat = permission;}
	public boolean getXMoveAllowedFlat(){return moveXallowedFlat;}
	public boolean getYMoveAllowedFlat(){return moveYallowedFlat;}

	public void setHasABall(boolean flag){hasAball = flag;}
	public void setSetForShot(boolean flag){setForShot = flag;}
	public void setShotCancelation(boolean flag){shotCancelation = flag;}
	public boolean getHasABall(){return hasAball;}
	public void setScore(int scored){score = scored;}
	public int getScore(){return score;}
	public double getPlayerAngle(){return playerAngle;}
	public double getStaminaForceFactor() {return staminaForceFactor;}
	public double getStamina(){return stamina;}
	public double getStaminaInitial(){return staminaInitial;}
	public boolean getCallBall(){return callBall;}
	public double getPlayerMass() { return playerMass; }
	public void setPlayerMass(double playerMass) { this.playerMass = playerMass; }
	public double getFootMass() { return footMass; }
	public void setFootMass(double footMass) { this.footMass = footMass; }
