...
public class Ball extends Creature {
	...
	//FLIGHT PARAMETERS
	private double Fs = 10+(Math.random()*990); // N, force of the kick that starts the game
	private double forceMax = 3500; //N, maximum force generated by leg muscles during steady shot
	...
	private double forceMaxTime = 3; //seconds to load maximum value of force (precision vs speed)
	private double w = 0; // Hz, angular velocity of the ball, max 20 ((Carlos-7Hz)
	private double wMax = 8;
	private double wMaxTime = 1.5; //seconds to load maximum value of w (precision vs speed)
	...
	private double ro = 1.168; // kg/m^3 air density
	...
	private double tk = 0.0086; //s, foot-ball time of contact 8.6-12ms!
	...
	public void setFs(double val){Fs=val;}
	...
	public void calculatePath(){
		...
		if (dx>0 && dy>0) tempAngle = Math.atan(dx/dy);
		if (dx>0 && dy==0) tempAngle = 0.5*Math.PI;
		if (dx>0 && dy<0) tempAngle = Math.atan(dx/dy)+Math.PI;
		if (dx==0 && dy<0) tempAngle = Math.PI;
		if (dx<0 && dy<0) tempAngle = Math.atan(dx/dy)+Math.PI;
		if (dx<0 && dy==0) tempAngle = 1.5*Math.PI;
		if (dx<0 && dy>0) tempAngle = Math.atan(dx/dy);
		...
		if (V<0.25 && V>-0.25) {flagMovement = false; V=0;} //ball stops if V<0.25m/s
		...
		if (Math.abs(w)<0.05) w=0; //rotation stops if w<0.05 round per s
	...
	private void roll(){
		...
		if (caughtBy == 0){
			...
			if ((V>13)||(V==0))rollingBall.setPeriod(100000);
			if (V<13) rotationAngle = -tempAngle-Math.PI;
			if (V>=13) rotationAngle -= 2*w*Math.PI/BallFPS;
		}
		if (caughtBy != 0) V=0;	
		if (caughtBy == 1){
			rollingBall.setPeriod(10000);
			if ((currX!=prevX)||(currY!=prevY))rollingBall.setPeriod(100-50*speed);//tu nie dzia?a, trzeba sprawdzi? drybling
		...
		if ((caughtBy == 0)&&(V<0.5))rollingBall.setPeriod(900000);
		rollingBall.tick();
	}
	...
	public double getXPrev2(){return xPrev2;}
	public void setXPrev2(double val){xPrev2=val;}
	public double getYPrev2(){return yPrev2;}
	public void setYPrev2(double val){yPrev2=val;}
	
	public double getTX(){return tx;}
	public void setTX(double val){tx=val;}
	public double getTY(){return ty;}
	public void setTY(double val){ty=val;}

	public void setforceMax(double val){forceMax=val;}
	public double getforceMax(){return forceMax;}
	public void setforceMaxTime(double val){forceMaxTime=val;}
	public double getforceMaxTime(){return forceMaxTime;}

	public double getV(){return V;}
	public void setV(double val){V=val;}

	public double getTempAngle(){return tempAngle;}
	public void setAngle(double val){angle=val;}
	public double getAngle(){return angle;}
	public void setAngleMaxTime(double val){angleMaxTime=val;}
	public double getAngleMaxTime(){return angleMaxTime;}

	public double getW(){return w;}
	public void setW(double val){w=val;}
	public double getwMax(){return wMax;}
	public void setwMax(double val){wMax=val;}
	public double getwMaxTime(){return wMaxTime;}
	public void setwMaxTime(double val){wMaxTime=val;}

	public double getTk(){return tk;}
	public double getMass(){return mass;}

	public double getRatio(){return ratio;}
	public void setCaughtBy(int indicator){caughtBy = indicator;}
	public int getCaughtBy(){return caughtBy;}
	
	public boolean getScored1(){return scored1;}
	public void setScored1(boolean val){scored1=val;}
	public boolean getScored2(){return scored2;}
	public void setScored2(boolean val){scored2=val;}
	public boolean getDribbleAllowed(){return dribbleAllowed;}
	public void setDribbleAllowed(boolean val){dribbleAllowed=val;}
	
	public void setFlagMovement(boolean flag){flagMovement = flag;}
